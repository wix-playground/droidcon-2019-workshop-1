...
export default class CharacterListItem extends React.PureComponent {
	...
	render() {
	    let img = this.props.detail.thumbnail.path+'/portrait_medium.'+this.props.detail.thumbnail.extension
	    let name = this.props.detail.name
	    let desc = this.props.detail.description?this.props.detail.description : 'no description'
		return (
		    <Card row height={150} style={styles.card}>
              ...
              <View padding-20 flex>
                ...
                <View row style={styles.btn}>
                	<Button text90 link red10 iconSource={deleteIcon} label="Remove" onPress={()=>this.props.onRemoveItem(this.props.detail.id)}/>
              	</View>
